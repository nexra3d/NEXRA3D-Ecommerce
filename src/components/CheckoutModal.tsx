        ...
        const res = await apiFetch('/api/shipping/estimate', {
          ...
          body: JSON.stringify({
            ...
            items: safeCartItems.map((item) => ({
              ...
              dimensions: (item.product?.length && item.product?.width && item.product?.height) ? {
    ...
    try {
      const checkoutRes = await apiFetch('/api/checkout', {
        ...
        body: JSON.stringify({
          ...
          shippingFee,
          fulfillmentMethod: selectedShippingOption?.id === 'pickup-store' ? 'STORE_PICKUP' : 'HOME_DELIVERY',
